...
WordPress を経由せず、`https://my-career.co.jp/subsidy-plan/` でアクセスできる静的ページとして公開します。
...
「実在するファイルまたはディレクトリにはリライトを適用しない」という意味なので、ドキュメントルート直下に `subsidy-plan/` フォルダを作って `index.html` を置けば、WordPress に取られず静的HTMLがそのまま配信されます。
...
直接URL: https://raw.githubusercontent.com/onotaisei-create/mycareer-subsidy-plan-mock/main/static/index.html
...
5. 「新規フォルダ作成」で `subsidy-plan` という名前のフォルダを作る
6. 作った `subsidy-plan/` フォルダに入って、「アップロード」で `index.html` を選択

#### 方法B: FTPクライアント（Cyberduck 等）

1. お名前.com Navi でFTPアカウント情報を取得
2. Cyberduck で SFTP 接続
3. ドキュメントルートに移動 → `subsidy-plan/` フォルダ作成 → `index.html` をドロップ
...
```
https://my-career.co.jp/subsidy-plan/
```
...
- Web Filer で `subsidy-plan/index.html` のパスを確認
